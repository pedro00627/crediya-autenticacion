...
public class UserValidator {
    ...
    public Mono<User> validateUser(final User user) {
        return Mono.when(
                        this.validateSalaryRange(user),
                        this.validateRoleExistence(user),
                        this.validateEmailUniqueness(user)
                )
                .then(Mono.just(user));
    }

    private Mono<Void> validateSalaryRange(final User user) {
    ...
    private Mono<Void> validateRoleExistence(final User user) {
        if (null == user.roleId()) {
            return Mono.empty();
        }
        return this.roleRepository.existsById(user.roleId())
    ...
    private Mono<Void> validateEmailUniqueness(final User user) {
        return this.userRepository.existByEmail(user.email())
